...
var BindingTableColumns = struct {
	ID            string
	CreatedAt     string
	UpdatedAt     string
	DeletedAt     string
	...
	ID:            "bindings.id",
	CreatedAt:     "bindings.created_at",
	UpdatedAt:     "bindings.updated_at",
	DeletedAt:     "bindings.deleted_at",
	LoginType:     "bindings.login_type",
	IdentifyToken: "bindings.identify_token",
	Mail:          "bindings.mail",
...
type bindingR struct {
	App *Application `boil:"App" json:"App" toml:"App" yaml:"App"`
}
...
func (r *bindingR) GetApp() *Application {
	if r == nil {
		return nil
	}
	return r.App
...
var (
	...
	bindingGeneratedColumns      = []string{}
)

type (
	// BindingSlice is an alias for a slice of pointers to Binding.
	// This should almost always be used instead of []Binding.
...
func (bindingL) LoadApp(e boil.Executor, singular bool, maybeBinding interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeBinding.(*Binding)
		if !ok {
			object = new(Binding)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeBinding)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeBinding))
			}
		}
	} else {
		s, ok := maybeBinding.(*[]*Binding)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeBinding)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeBinding))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`applications.name in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func Bindings(mods ...qm.QueryMod) bindingQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`bindings`.*"})
	}

	return bindingQuery{q}
}
...
func FindBinding(exec boil.Executor, iD uint, selectCols ...string) (*Binding, error) {
	...
	if err = bindingObj.doAfterSelectHooks(exec); err != nil {
		return bindingObj, err
	}
...
// Exists checks if the Binding row exists.
func (o *Binding) Exists(exec boil.Executor) (bool, error) {
	return BindingExists(exec, o.ID)
}
