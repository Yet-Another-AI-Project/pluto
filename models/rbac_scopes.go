...
var RbacScopeTableColumns = struct {
	ID        string
	CreatedAt string
	UpdatedAt string
	DeletedAt string
	...
	ID:        "rbac_scopes.id",
	CreatedAt: "rbac_scopes.created_at",
	UpdatedAt: "rbac_scopes.updated_at",
	DeletedAt: "rbac_scopes.deleted_at",
	Name:      "rbac_scopes.name",
	AppID:     "rbac_scopes.app_id",
}
...
var (
	...
	rbacScopeGeneratedColumns      = []string{}
)

type (
	// RbacScopeSlice is an alias for a slice of pointers to RbacScope.
	// This should almost always be used instead of []RbacScope.
...
func RbacScopes(mods ...qm.QueryMod) rbacScopeQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`rbac_scopes`.*"})
	}

	return rbacScopeQuery{q}
}
...
func FindRbacScope(exec boil.Executor, iD uint, selectCols ...string) (*RbacScope, error) {
	...
	if err = rbacScopeObj.doAfterSelectHooks(exec); err != nil {
		return rbacScopeObj, err
	}
...
// Exists checks if the RbacScope row exists.
func (o *RbacScope) Exists(exec boil.Executor) (bool, error) {
	return RbacScopeExists(exec, o.ID)
}
