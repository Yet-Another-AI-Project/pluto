...
var DeviceAppTableColumns = struct {
	ID        string
	CreatedAt string
	UpdatedAt string
	DeletedAt string
	...
	ID:        "device_apps.id",
	CreatedAt: "device_apps.created_at",
	UpdatedAt: "device_apps.updated_at",
	DeletedAt: "device_apps.deleted_at",
	DeviceID:  "device_apps.device_id",
	AppID:     "device_apps.app_id",
}
...
var (
	...
	deviceAppGeneratedColumns      = []string{}
)

type (
	// DeviceAppSlice is an alias for a slice of pointers to DeviceApp.
	// This should almost always be used instead of []DeviceApp.
...
func DeviceApps(mods ...qm.QueryMod) deviceAppQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`device_apps`.*"})
	}

	return deviceAppQuery{q}
}
...
func FindDeviceApp(exec boil.Executor, iD uint, selectCols ...string) (*DeviceApp, error) {
	...
	if err = deviceAppObj.doAfterSelectHooks(exec); err != nil {
		return deviceAppObj, err
	}
...
// Exists checks if the DeviceApp row exists.
func (o *DeviceApp) Exists(exec boil.Executor) (bool, error) {
	return DeviceAppExists(exec, o.ID)
}
