...
var SaltTableColumns = struct {
	...
	ID:        "salts.id",
	CreatedAt: "salts.created_at",
	UpdatedAt: "salts.updated_at",
	DeletedAt: "salts.deleted_at",
	UserID:    "salts.user_id",
	Salt:      "salts.salt",
}
...
var (
	...
	saltGeneratedColumns      = []string{}
)

type (
	// SaltSlice is an alias for a slice of pointers to Salt.
	// This should almost always be used instead of []Salt.
	SaltSlice []*Salt
...
func Salts(mods ...qm.QueryMod) saltQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`salts`.*"})
	}
...
func FindSalt(exec boil.Executor, iD uint, selectCols ...string) (*Salt, error) {
	...
	if err = saltObj.doAfterSelectHooks(exec); err != nil {
		return saltObj, err
	}
...
// Exists checks if the Salt row exists.
func (o *Salt) Exists(exec boil.Executor) (bool, error) {
	return SaltExists(exec, o.ID)
}
